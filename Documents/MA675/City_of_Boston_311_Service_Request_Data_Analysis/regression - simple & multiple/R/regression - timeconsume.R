...
# multiple lm
summary(lm(Public.Works.Department ~ male + female + factor(timecons$black.african)))
summary(lm(Inspectional.Services ~ income + factor(timecons$white) + factor(timecons$black.african)))
summary(lm(Parks...Recreation.Department ~ income + factor(timecons$white)))
summary(lm(Property.Management ~ density+factor(timecons$age)+density*factor(timecons$age)))
# interaction between age and density shows up
summary(lm(Transportation...Traffic.Division ~ area + density + age.over.18 + male + female 
           + factor(timecons$black.african)))